...
<!--  <el-dialog width="90%" title="信息填写" :visible.sync="dialogSeniorVisible">-->
    <div style="display:flex;overflow: auto;width: 100%">
       <el-card class="box-card">
         <el-form  ref="seniorForm" :rules="rules" :model="form" style="padding: 20px 30px;"  label-width="200px" v-show="showInfo">
...
<!--           </el-form-item>-->
           ...
           <el-descriptions class="margin-top" title="A.2 评估对象基本信息" :column="2" size="medium">
             <el-descriptions-item label="A.2.1 姓名"><el-tag type="primary">{{form.assessedName}}</el-tag></el-descriptions-item>
             <el-descriptions-item label="A.2.2 身份证号">{{form.identityCard}}</el-descriptions-item>
             <el-descriptions-item label="A.2.3 性别">{{form.gender}}</el-descriptions-item>
             <el-descriptions-item label="A.2.4 出生日期">{{form.birthday}}</el-descriptions-item>
             <el-descriptions-item label="A.2.5 年龄">{{form.age}}</el-descriptions-item>
             <el-descriptions-item label="A.2.6 社保卡号">{{form.socialCard}}</el-descriptions-item>

             <el-descriptions-item label="A.2.7 民族">
               <div v-for="item in this.nationList">
                 <span v-if="form.nationality === item.info.toString()">{{item.info}}</span>
               </div>
             </el-descriptions-item>
             <el-descriptions-item label="A.2.8 文化程度">
               <div v-for="item in this.educationList">
                 <span v-if="form.educationLevel === item.value">{{item.label}}</span>
               </div>
             </el-descriptions-item>
             <el-descriptions-item label="A.2.9 宗教信仰">
               <div v-for="item in religionList">
                 <span v-if="form.religion === item.value">{{ item.info }}</span>
               </div>
             </el-descriptions-item>
             <el-descriptions-item label="A.2.10 婚姻状况">
               <div v-for="item in maritalList">
                 <span v-if="form.maritalStatus === item.value">{{item.label}}</span>
               </div>
             </el-descriptions-item>
             <el-descriptions-item label="A.2.11 居住情况">
               <div v-model="form.residenceStatus" v-for="item in residenceList">
                 <span v-if="form.residenceStatus === item.value" >{{item.label}}</span>
               </div>
             </el-descriptions-item>
             <el-descriptions-item label="A.2.12 医疗费用支付方式">
               <div v-for="item in medicalPaymentsList" style="display: flex">
                 <el-tag v-if="tip === item.value" v-for="tip in medicalPayments" >{{item.label}}</el-tag>&nbsp;
               </div>
             </el-descriptions-item>
             <el-descriptions-item label="A.2.13 经济来源">
               <div v-for="item in financialSourceList" style="display: flex">
                 <el-tag v-if="tip === item.value" v-for="tip in financialSource">{{item.label}}</el-tag>&nbsp;
               </div>
             </el-descriptions-item>
           </el-descriptions>
            <br>
...
<!--               <div v-for="item in this.educationList">-->
<!--                 <span v-if="form.educationLevel === item.value">{{item.info}}</span>-->
<!--               </div>-->
<!--             </el-form-item>-->
<!--           </div>-->
<!--           <div style="display:flex;">-->
...
<!--               <div v-for="item in medicalPaymentsList" style="display: flex">-->
<!--                 <div v-for="tip in medicalPayments">-->
<!--                   <el-tag v-if="tip === item.value">{{item.label}}</el-tag>-->
<!--                 </div>-->
<!--               </div>-->
<!--           </el-form-item>-->
<!--           <el-form-item label="A.2.13 经济来源 "  prop="financialSource" style="margin-bottom: 20px;flex: 1">-->
<!--             <div v-for="item in financialSourceList" style="display: flex">-->
<!--               <div v-for="tip in financialSource">-->
<!--                 <el-tag v-if="tip === item.value">{{item.label}}</el-tag>-->
<!--               </div>-->
<!--             </div>-->
<!--           </el-form-item>-->
             <el-descriptions class="margin-top" title="A.2.14 疾病诊断" :column="4" size="medium" border>
               <el-descriptions-item>
                 <template slot="label">
                   A.2.14.1 痴呆
                 </template>
                 <div v-model="form.stupidStatus" v-for="item in stupidStatusList">
                   <span v-if="form.stupidStatus === item.value" >{{item.label}}</span>
                 </div>
               </el-descriptions-item>
               <el-descriptions-item>
                 <template slot="label">
                 ...
                 <div v-model="form.spiritDisease" v-for="item in spiritDiseaseList">
                   <span v-if="form.spiritDisease === item.value" >{{item.label}}</span>
                 </div>
               </el-descriptions-item>
               <el-descriptions-item>
                 <template slot="label">
             ...
             <br>
             <el-descriptions class="margin-top" title="A.2.15 近 30 天内意外事件" :column="4" size="medium" border>
<!--               <template slot="extra">-->
...
<!--               </template>-->
               <el-descriptions-item>
                 ...
                 <div v-model="form.fallDown" v-for="item in accidentList">
                   <span v-if="form.fallDown === item.value" >{{item.label}}</span>
                 </div>
               </el-descriptions-item>
               <el-descriptions-item>
                 <template slot="label">
                 ...
                 <div v-model="form.beLost" v-for="item in accidentList">
                   <span v-if="form.beLost === item.value" >{{item.label}}</span>
                 </div>
               </el-descriptions-item>
               <el-descriptions-item>
                 <template slot="label">
                 ...
                 <div v-model="form.choke" v-for="item in accidentList">
                   <span v-if="form.choke === item.value" >{{item.label}}</span>
                 </div>
               </el-descriptions-item>
               <el-descriptions-item>
                 <template slot="label">
                 ...
                 <div v-model="form.suicide" v-for="item in accidentList">
                   <span v-if="form.suicide === item.value" >{{item.label}}</span>
                 </div>
               </el-descriptions-item>
               <el-descriptions-item>
                 <template slot="label">
           ...
           <br>
           <el-descriptions class="margin-top" title="A.3 信息提供者及联系人信息" :column="2" size="medium" border>
             <!--               <template slot="extra">-->
             <!--                 <el-button type="primary" size="small">操作</el-button>-->
             <!--               </template>-->
             <el-descriptions-item>
               ...
               {{form.informationProviderName}}
             </el-descriptions-item>
             <el-descriptions-item>
               <template slot="label">
                 A.3.2 信息提供者与老人的关系
               </template>
               <div v-model="form.informationProviderAndSeniorAssociation" v-for="item in informationProviderAndSeniorAssociationList">
                 <span v-if="form.informationProviderAndSeniorAssociation === item.value" >{{item.label}}</span>
               </div>
             </el-descriptions-item>
             <el-descriptions-item>
               <template slot="label">
         ...
         <el-form  ref="seniorForm" :rules="rules" :model="form" style="padding: 20px 30px;"  label-width="200px" v-show="editInfo">

           <el-form-item label="A.2.1 姓名" prop="assessedName" style="margin-bottom: 20px">
             <el-input  maxlength="20" show-word-limit size="medium" v-model="form.assessedName" placeholder="请输入真实姓名" style="width: 90%" disabled></el-input>
           </el-form-item>
           <br>
           <el-form-item label="A.2.2 身份证号" prop="identityCard" style="margin-bottom: 20px">
             <el-input :maxlength="identityCardMaxLength" show-word-limit @input="identityCardChange"
                       size="medium" v-model="form.identityCard" placeholder="请输入内容"  autocomplete="off" style="width: 90%" disabled>
                     <template slot="prepend" style="width: 100%;">
                       <el-select class="select_identityCard" v-model="select" slot="prepend" placeholder="请选择" @change="identityCardNumChange" disabled>
           ...
           <div style="display:flex;">
             <el-form-item label="A.2.7 民族" prop="nationality" style="margin-bottom: 20px;flex: 1">
               <el-select v-model="form.nationality" filterable placeholder="请选择民族" style="width: 75%;">
                 <el-option v-for="item in nationList" :key="item.info" :label="item.info" :value="item.info"></el-option>
               </el-select>
             </el-form-item>
             <el-form-item label="A.2.8 文化程度" prop="educationLevel" style="margin-bottom: 20px;flex: 1">
         ...
         <div style="text-align: center;margin-top: 20px">
           ...
           <el-button v-if="editInfo === false && isEdit === 0" type="primary" @click="edit" style="height: 40px;width: 100px;margin-right: 30px;float: right"><span style="font-size: 15px"><i class="el-icon-edit-outline"></i>&nbsp;编辑</span></el-button>
           <el-button style="height: 40px;width: 100px;margin-left: 10px" type="primary" v-if="editInfo === true && isEdit === 0" @click="save"><span style="font-size: 15px"><i class="el-icon-check"></i>&nbsp;保存</span></el-button>
         </div>
         <br>
         <br>
       </el-card>
      ...
      if (Object.keys(user).length > 0) {
        this.request.get("/senior/person/" + user.id).then(res => {
          if (res.code === '200') {
            console.log(res)
            if (res.data === null) {
              this.form.assessedName = user.nickname
              this.form.identityCard = user.identityCard
              this.identityCardChange()
            } else {
              this.form = res.data;
              this.medicalPayments = res.data.medicalPayments.toString().split(',')
              this.financialSource = res.data.financialSource.toString().split(',')
            }
          } else {
            this.$message.error("系统错误")
          }
